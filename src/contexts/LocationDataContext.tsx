...
interface LocationDataContextType {
  ...
  updateLocationData: (lat: number, lng: number, name?: string) => Promise<void>;
...
export const LocationDataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [sunTimesCache, setSunTimesCache] = useState<Map<string, ProcessedSunTimes>>(new Map());
  ...
  const getSunTimesForDate = useCallback((date: Date): ProcessedSunTimes | null => {
  ...
  const fetchSunTimes = useCallback(async (date: Date) => {
    if (!location || !timezoneInfo.timezone) {
      console.warn('Cannot fetch sun times: location or timezone not available');
      return;
    }
    ...
    try {
      ...
      const data = await getSunTimes(location.latitude, location.longitude, dateKey);
